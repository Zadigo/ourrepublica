<template>
  <nav class="fixed top-0 left-0 h-(--navbar-height) w-full px-6 md:px-16 lg:px-24 xl:px-32 flex items-center justify-between z-20 bg-white text-gray-700 shadow-md transition-all">
    ...
    <ul class="md:flex hidden items-center gap-10">
      <li v-for="link in links" :key="link.name">
        <nuxt-link class="hover:text-gray-500/80 transition" :to="link.path">
          {{ link.name }}
        </nuxt-link>
      </li>
    </ul>
    ...
    <div class="mobile-menu absolute top-(--navbar-height) left-0 w-full bg-white p-6 hidden md:hidden">
      <ul class="flex flex-col space-y-4 text-lg">
        <li v-for="link in links" :key="link.name">
          <nuxt-link :to="link.path" class="text-sm">
            {{ link.name }}
          </nuxt-link>
        </li>
      </ul>
